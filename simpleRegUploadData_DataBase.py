...
def excel(): 
    ...
    sheet.cell(row=1, column=8).value = "Roll Number"
...
def insert(): 
    ...
    if (name_field.get() == "" and
        ...
        sheet.cell(row=current_row + 1, column=8).value = roll_number_field.get() 
	# Data inserting into database Start
	form=str(roll_number_field.get())
	if regDetails.get() == 1 and examDetails.get() == 1:
		insert_reg_record = "INSERT INTO Registration(FORM_NUMBER,ROLL_NUMBER,NAME,COURSE,SEMISTER,CONTACT_NUMBER,EMAIL_ID,ADDRESS) VALUES('%s','%s','%s','%s','%s','%s','%s','%s');"%(form_no_field.get(),roll_number_field.get(),name_field.get(),course_field.get(),sem_field.get(),contact_no_field.get(),email_id_field.get(),address_field.get())
		insert_exam_record = "INSERT INTO Exam(FORM_NUMBER,ROLL_NUMBER,NAME,COURSE,SEMISTER,CONTACT_NUMBER,EMAIL_ID,ADDRESS) VALUES('%s','%s','%s','%s','%s','%s','%s','%s');"%(form_no_field.get(),roll_number_field.get(),name_field.get(),course_field.get(),sem_field.get(),contact_no_field.get(),email_id_field.get(),address_field.get())
